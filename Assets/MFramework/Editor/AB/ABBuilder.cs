...
namespace MFramework
{
    ...
    public class ABBuilder
    {
        ...
        public static readonly string TempPath = $"{MPathCache.TEMP_ASSET_PATH}/AB";
        ...
        {
            string normalized = fullPath.ReplaceSlash();
            string prefix = $"{MPathCache.PC_ROOT_PATH.ReplaceSlash()}/";
            return normalized.StartsWith(prefix, StringComparison.Ordinal)
                ? normalized.Substring(prefix.Length)
                : normalized;
        }

        private static void ImportManifestAsset(string assetPath)
        {
        ...
        internal static void BuildInternal(string buildSettingPath)
        {
            ...
            MLog.Default?.D($"打包完成{ms_BuildProfiler}");
        }
        ...
        private static Dictionary<string, List<string>> CollectDependency(ICollection<string> files)
        {
            ...
            int segmentCount = 10;
            int segmentSize = Mathf.Max(1, files.Count / segmentCount);
            for (int i = 0; i < fileList.Count; i++)
            {
                if (i % segmentSize == 0 || i == files.Count - 1)
                {
                ...
                // Tip：会比原来的files更多，因为存在依赖文件files的依赖依赖的依赖
        ...
        public static bool HasCycle(Dictionary<string, List<string>> dependencyDic)
        {
            ...
            {
                MLog.Default?.D("检测到的循环依赖路径有：");
                foreach (var cycle in allCycles)
                {
                    MLog.Default?.D(string.Join(" -> ", cycle));
                }
                return true;
            }
            return false;
        ...
        private static Dictionary<string, List<string>> CollectBundle(BuildSetting buildSetting, Dictionary<string, ResourceType> assetDic, Dictionary<string, List<string>> dependencyDic)
        {
            ...
            if (notInRuleList.Count > 0)
            {
                ...
                EditorUtility.ClearProgressBar();
                MLog.Default?.E($"存在意外或后缀不匹配的资源：   Tip：如果是Prefab，需要先获取其引用{message}");
            }
        ...
        private static void GenerateManifest(Dictionary<string, ResourceType> assetDic, Dictionary<string, List<string>> bundleDic, Dictionary<string, List<string>> dependencyDic)
        {
            ...
            #region 生成资源描述信息
            {
                ...
                {
                    EditorUtility.ClearProgressBar();
                    MLog.Default?.E($"资源个数超出{ushort.MaxValue}");
                }
            ...
            #region 生成bundle描述信息
            {
                ...
                foreach (var kv in bundleDic)
                {
                    ...
                    for (int i = 0; i < assets.Count; i++)
                    {
                        ...
                        bundleBw.Write(assetId); // 4.资源id（用id替换字符串可以节省内存）
                    }
                }
            ...
            #region 生成资源依赖描述信息
            {
                ...
                foreach (var kv in dependencyDic)
                {
                    ...
                    {
                        EditorUtility.ClearProgressBar();
                        MLog.Default?.E($"资源{assetUrl}的依赖超出一个字节上限:{byte.MaxValue}");
                    }
        ...
        private static AssetBundleManifest BuildBundle(Dictionary<string, List<string>> bundleDic)
        {
            ...
            // 删除所有.manifest文件（不需要，有总.manifest文件）
            MPathUtility.DeleteFileWithExtension(BuildPath, BUNDLE_MANIFEST_SUFFIX);
        ...
        private static void BuildManifest()
        {
            ...
            string TempBuildPath = $"{MPathCache.TEMP_ROOT_PATH}/AB";
            if (!Directory.Exists(TempBuildPath)) Directory.CreateDirectory(TempBuildPath);

            string resourceAssetPath = ToAssetPath(ResourceBYTEPath);
            string bundleAssetPath = ToAssetPath(BundleBYTEPath);
            string dependencyAssetPath = ToAssetPath(DependencyBYTEPath);
            // 需强制导入
            ImportManifestAsset(resourceAssetPath);
            ImportManifestAsset(bundleAssetPath);
            ImportManifestAsset(dependencyAssetPath);
            // 创建manifest文件
        ...
        private static void BuildMD5(string rootPath)
        {
            ...
            File.WriteAllText($"{rootPath}/{MPathCache.AB_INFO_FILE_NAME}", sb.ToString());
