...
public class Collectable : MonoBehaviour
{
    ...
    protected virtual void Update()
    {
        if (!m_vanished)
        {
            HandleGhosting();//幽灵时间(出现后幽灵时间内不可拾取)
            HandleLifeTime();//生命周期(出现后生命周期过后消失)
    ...
    public virtual void Collect(Player player)
    {
        if (!m_vanished && !m_ghosting)
        {
            ...
            {
                MCoroutineManager.Instance.BeginCoroutineNoRecord(QuickShowRoutine());
            }

            MCoroutineManager.Instance.BeginCoroutineNoRecord(CollectRoutine(player));
        }
    }
